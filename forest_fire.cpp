                   ...
                   int& steps, bool &fireReachedBottom)
{
    ...
    while (fireStillBurning) {
        ...
        int globalBurning = 0;
        // We use MPI_MAX to see if any rank has burning (1)
        MPI_Allreduce(&localBurning, &globalBurning, 1, MPI_INT, MPI_MAX, MPI_COMM_WORLD);
        ...
        int globalBottom = 0;
        MPI_Allreduce(&localBottom, &globalBottom, 1, MPI_INT, MPI_MAX, MPI_COMM_WORLD);
        if (globalBottom)
